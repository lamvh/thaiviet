...
**Image sources allowed by validation.** `src/lib/image-url.ts` (`isRenderableImageUrl`) is the single rule shared by `content-schema.ts` and `templates/validate-page.ts`: an image field accepts either an `https://` URL (Storage, any CDN) **or** a root-relative `/path` pointing at a file committed under `public/` — the CSP allows `img-src 'self'`. `http://`, protocol-relative `//host/…`, `blob:` and bare filenames are still rejected. Bundled assets are the right choice for artwork that ships with the build (the homepage hero banner `public/images/hero-banner.webp`, and the three service images `interior-painting.webp` / `exterior-painting.webp` / `roof-maintenance.webp` used by the Interior, Exterior and Roof services); everything the client swaps day-to-day should go through Storage. Keep bundled images `.webp` and kebab-case — a space in a filename has to be percent-encoded in every URL that references it. Note the bundled path is resolved against Vite's `base`, so it assumes the site is served from a domain root. Video URLs (`home.video.src` / `poster`) remain `https://`-only.
...
Admin edits write themselves back — there is **no Save/Publish button**. `AdminContentProvider` (`pages/admin/admin-content-store.tsx`) runs a debounced effect: any change to `content` arms a timer for `AUTOSAVE_DELAY_MS` (1500 ms) and each further edit restarts it, so continuous typing costs **one** row write when typing pauses, not one per keystroke. `PublishBar.tsx` is now a status indicator only — *Saving…* → *Saved — live now*, or a sticky red error.

Whether a save is armed is decided by the pure `shouldScheduleAutosave(state, lastAttempted)` (exported, unit-tested in `__tests__/autosave-schedule.test.ts`) so the retry rules are testable without React. Three rules: nothing unsaved → no write; a write already in flight → no second write; and **a rejected payload never retries on a timer** — `lastAttempted` holds the exact content object that failed, and since content is replaced immutably, reference equality answers "has the editor actually changed something since?". Without that guard an invalid field or a concurrency abort would re-fire every 1.5 s forever.

Consequences to keep in mind when touching this area:
- **Validation failures are a status, not a toast.** `publish()` dispatches `PUBLISH_ERROR` (sticky) instead of `TOAST` (fades in 2.2 s) — with no button click to tie it to, a fading message would let an editor keep typing into content that is silently no longer saving. Fix the flagged field and the next edit re-arms autosave automatically.
- **The concurrent-edit guard still aborts** when another admin moved the row (`remoteUpdatedAt` mismatch). That error is terminal for the session: `remoteUpdatedAt` never advances, so every later save fails the same way. Reload to pick up their version. Autosave does not merge.
- **Wizards and the edit drawer keep their one commit step** (*Add service* / *Save changes* / *Apply*). Their draft (`compose` / `serviceCompose` / `draft`) lives outside `content` on purpose — a half-filled service must not go live. Only once it lands in `content` does autosave pick it up.
- **`beforeunload` stays.** The unsaved window is short (debounce + one request) but not zero.
- No success toast on save — the indicator covers it; a toast per edit burst is noise.
...
- **Temporary (remove after the content row is republished):** `content-defaults.ts` rewrites one retired image URL (`project.vinapage.com/…/banner.png`, still stored in the live row at `home.hero.image` and `home.video.poster`) to the bundled `/images/hero-banner.webp` on every load. Without it the site rendered the new banner and then swapped back to the old one as soon as the remote row arrived. It is a find-and-replace on that exact URL, not a forced field value, so any other image an editor picks passes through. Admin loads through the same path, so the next **Publish** persists the corrected URL — after that, delete `rewriteRetiredBanner` + its two constants and the matching tests in `__tests__/content-defaults.test.ts`.
- Homepage hero treatment: the banner (`home.hero.image`, admin → **Homepage** → *Background image URL*) is full-bleed `object-cover`, focal point biased right (`object-[68%_center]`) on phones so the house stays in frame when `cover` crops the sides, and centred from `md` up. Over it sits **one** scrim div — vertical on mobile (copy spans the full width), horizontal on desktop (copy sits in the left column, photo stays in full colour on the right). The copy is **light-on-dark**: white headline, `text-primary-bright` accent, `text-white/85` body, solid red badge chip, and the certification PNGs sit on `bg-white/90` chips because their artwork is dark on transparent. Swapping in a much lighter or darker photo means re-checking those scrim stops.
- Tailwind colour tokens: the palette in `tailwind.config.js` is Material-3 derived. Two keys are additions, not from the generated ramp — `surface` (`#fcf9f4`, the same cream as `background` and the `body` rule in `index.css`) and `primary-bright` (`#ff3b30`, the brand red lifted to stay legible on a dark photo scrim; display text only). `surface` in particular is load-bearing: ~80 `bg-surface` / `from-surface/NN` classes across the app emit nothing without it, which silently drops scrims and panels.
