...
- **Temporary (pinned service artwork):** `content-defaults.ts` also force-pins the images of three services — `interior` → `/images/interior-painting.webp`, `exterior` → `/images/exterior-painting.webp`, `roof` → `/images/roof-maintenance.webp` (`PINNED_SERVICE_IMAGES` + `pinServiceImages`). Unlike the banner rewrite this is a **forced value, not a find-and-replace**: it is applied to both `image` (card) and `page.meta.heroImg` (detail hero) on every load, so the stored content row cannot override it and **these three images are not editable from admin** while the pin exists. The bundled `site-content.json` already holds the same paths, so handing them back to the CMS is just deleting `PINNED_SERVICE_IMAGES`, `pinServiceImages`, its two call sites and the matching tests. The other five services stay fully CMS-driven.
